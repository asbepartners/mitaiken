...
export interface TriedRecordDraft {
  timing: Timing;
  memo?: string;
  photoUrl?: string;
}
...
const inputClass =
  "min-w-0 max-w-full w-full rounded-2xl border border-green-100 bg-ivory px-4 py-3 text-base text-ink focus:border-green-700 focus:outline-none";
      ...
      <div className="relative max-h-[92dvh] min-w-0 w-full max-w-sm overflow-y-auto rounded-t-3xl bg-paper px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-3 shadow-[0_-4px_24px_rgba(44,38,32,0.15)] sm:rounded-3xl">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h2 className="mt-1 line-clamp-2 text-xl font-bold leading-snug text-green-950">
              {experienceTitle}
            </h2>
          </div>
          <button
            ...
            onClick={onCancel}
            aria-label="閉じる"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-ivory-deep text-xl leading-none text-green-950"
          >
            ×
          </button>
        </div>
            ...
            {([
              ...
              <button
                ...
                className={`py-2.5 text-sm font-bold transition-colors ${
                  !unknown && mode === value
                    ? "border border-coral-400 bg-coral-100 text-green-950"
                    : "text-green-900"
                }`}
              >
                {label}
          ...
          <div className="mt-1.5 text-right">
            <button
              ...
              className={`text-xs font-medium underline underline-offset-4 ${
                unknown ? "text-coral-500" : "text-green-800"
              }`}
            >
        ...
        <div className="mt-4">
          <label htmlFor="tried-memo" className="text-sm font-bold text-green-950">
            ひとことメモ <span className="font-normal text-ink-soft">（任意）</span>
          </label>
          <div className="relative mt-1.5">
            ...
            />
            <span className="absolute bottom-2 right-3 text-[10px] text-ink-soft">
              {memo.length}/100
            </span>
          </div>
        </div>
        ...
        <div className="mt-4">
          <p className="text-sm font-bold text-green-950">
            写真 <span className="font-normal text-ink-soft">（任意）</span>
          </p>
            ...
            {photoUrl ? (
              <div className="relative h-24 w-24 overflow-hidden rounded-2xl border border-green-100 bg-ivory">
                ...
                <button
                  ...
                  aria-label="写真を削除"
                  className="absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-paper/95 text-sm font-bold text-green-950 shadow"
                >
                  ×
                </button>
              </div>
        ...
        <div className="mt-5 flex gap-2">
          <button
            type="button"
            onClick={onCancel}
            className="flex-1 rounded-full border border-green-100 bg-paper py-3 text-sm font-medium text-ink-soft"
          >
            戻る
          </button>
          <button
            type="button"
            disabled={!canConfirm || processingPhoto}
            onClick={() =>
              onConfirm({
                timing,
            ...
            className="flex-1 rounded-full bg-green-800 py-3 text-sm font-bold text-paper transition-colors disabled:opacity-40"
          >
            決定
          </button>
        </div>
