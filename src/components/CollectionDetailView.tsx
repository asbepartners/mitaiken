...
export function CollectionDetailView(props: Props) {
  ...
  const displayedRecords = isCollection ? latestRecordsByTarget : records;
  ...
  return <div className="px-4 pb-6">
    ...
    <header className="mt-3 overflow-hidden rounded-3xl border border-green-100 bg-paper shadow-[0_2px_12px_rgba(44,38,32,0.06)]">
      <button type="button" onClick={() => setPreviewPhoto({ src: imageSource(experience.image, assetBase), alt: `${experience.title}の写真` })} aria-label={`${experience.title}の写真を拡大`} className="block h-44 w-full cursor-zoom-in overflow-hidden bg-ivory">
      ...
      <div className="p-5">
        ...
        {props.onEditExperience && <button type="button" onClick={props.onEditExperience} className="mt-4 min-h-11 w-full rounded-full border border-coral-300 bg-paper px-3 text-sm font-bold text-coral-500">リスト情報を編集</button>}
    ...
    {displayedRecords.length > 0 && <section className="mt-7 border-t border-green-100 pt-6"><h2 className="px-1 text-lg font-bold text-green-950">やってみた記録</h2><div className="mt-3"><ExperienceRecordTimeline experience={experience} records={displayedRecords} targets={targets} onOpenTarget={isCollection ? setSelectedTargetId : undefined} onPreviewPhoto={(src, alt) => setPreviewPhoto({ src, alt })} onEditRecord={props.onEditRecord} onDeleteRecord={(recordId) => { props.onDeleteRecord(recordId); setMenuId(null); }} openMenuId={menuId} onToggleMenu={(recordId) => setMenuId(menuId === recordId ? null : recordId)} /></div></section>}
