...
// Regression coverage for asbepartners/mitaiken#54 and its follow-up #70: a
// logged-out visitor can tap やってみたい♡ and fill in any form freely, but
// is asked to log in at the moment they try to save anything that's actually
// personal information they typed in -- both a やったことある record and a
// やってみたい詳細 (予定日/場所/同行者/メモ/参考URL) are gated this way
// (#70; previously only やったことある was gated). Every other spec in this
// suite inherits the authenticated storageState from global setup; this one
// deliberately runs with no session at all.
...
test.describe("未ログインの利用", () => {
  test("やってみたいのタップは自由にできるが、やってみたい詳細・やったことあるの保存はどちらもログインを求められる", async ({ page }) => {
    await page.goto("/app");
    ...
    // やってみたいリストの詳細シートは自由に開いて入力できる -- ここではまだ
    // ログイン画面が出ないこと
    ...
    // ログインをキャンセルしても、入力した内容は保持されたまま詳細シートに戻れること
    // (AuthSheetは詳細シートの上に重なって表示されるため、DOM順で最後に
    // 現れる「閉じる」ボタン=AuthSheet自身のものを狙う)
